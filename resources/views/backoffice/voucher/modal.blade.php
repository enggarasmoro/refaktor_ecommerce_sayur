        ...
        <div class="modal-content">
            ...
            <form id="form" class="parsley-voucher">
                ...
                <div class="modal-body p-4">
                    <?php $value = (isset($voucher->id))?$voucher->id:""; ?>
                    <input type="hidden" name="id" value="{{$value}}">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <?php $value = (isset($voucher->nama))?$voucher->nama:""; ?>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <?php $value = (isset($voucher->nominal))?$voucher->nominal:""; ?>
                                <label class="required font-weight-bold text-dark text-2">Nominal</label>
                                <input type="number" min="0"  id="nominal" name="nominal" class="form-control"  value="{{ $value }}" parsley-trigger="change" required>
                            </div>
                        </div>
                    </div><br>
                    <div class="row">
                        <div class="col-md-12">
                            <?php $value = (isset($voucher->status))?$voucher->status:""; ?>
...
<script type="text/javascript">
    ...
    $("#form").on("submit", (function(b) {
        ...
        var type = "{{$type}}";
        if (type == "simpan") {
            url = "{{route('configvoucher.create')}}"
        } else {
            url = "{{route('configvoucher.edit')}}"
        }
        $.ajax({
            url: url,
            type: "POST",
            ...
            success: function(data) {
                ...
                if (data.errorsvalidation) {
                    ...
                    Swal.fire({
                        ...
                        $('#config_voucher').DataTable().ajax.reload();
                    });
                } else {
                    $("#myModal").modal("hide");
                    Swal.fire({
